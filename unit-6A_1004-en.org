...
** Costs and production\NL (and III)
...
** The cost curves\NL in the short term
- The fixed cost does not depend on the production level:
  \[ \CF(Q) = r \Kbar \]
...
** Relationship between\NL $\PMg$, $\PMe$, $\CMg$ and $\CMe$ \NL (I)
...
** Relationship between\NL $\PMg$, $\PMe$, $\CMg$ and $\CMe$ \NL (II)
...
** Relationship between\NL $\PMg$, $\PMe$, $\CMg$ and $\CMe$ \NL (and III)
...
** Average variable cost\NL and marginal cost\NL curves
...
** The average fixed cost\NL curve
...
** The average cost curve\NL (and II)
