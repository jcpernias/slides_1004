...
- *Recta isocoste*: todas las combinaciones de factores que
  representan el mismo coste:
  #+begin_export latex
  \[ \Cbar = r K + w L \]
  #+end_export

- A menudo la escribimos como:
  #+begin_export latex
...
** Las demanda condicional de un factor

- La *demanda condicional de un factor* muestra la cantidad óptima del
  factor para cada nivel de producción:
  #+BEGIN_EXPORT LaTeX
    \begin{gather*}
      L = L(Q) \\
      K = K(Q)
...
- Las curvas $\CMeLP$ y $\CMgLP$ se cortan en el mínimo coste unitario
  a largo plazo.
...
- La curva de $\CMeLP$:
  ...
  + decrece cuando $\CMgLP < \CMeLP$.

  + crece cuando $\CMgLP > \CMeLP$.
...
- Cada curva de costes a corto tiene es tangente a la
  curva de costes a largo plazo en un punto.
