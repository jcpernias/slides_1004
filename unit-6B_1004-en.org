...
- *Isocost line*: all input combinations that represent the same cost:
  #+begin_export latex
  \[ \Cbar = r K + w L \]
  #+end_export
...
** The product expansion path of the
...
** Conditional demand of and input

- The *conditional demand of an input* shows the optimal quantity of
  that input for each level of production:
  #+BEGIN_EXPORT LaTeX
    \begin{gather*}
      L = L(Q) \\
      K = K(Q)
...
- The $\CMeLP$ and $\CMgLP$ curves cross at the minimum unit cost in
  the long-run.
...
- Different output levels can only be reached by altering the level of
  $L$.
