...
- La inversa de la función de producto total muestra el trabajo que se
  requiere para obtener un determinado nivel de producción:
  #+begin_export latex
  \[ L = f^{-1}(Q) \]
  #+end_export
...
- El coste variable medio:
  ...
  + decrece cuando $\CMg < \CVMe$.

  + crece cuando $\CMg > \CVMe$.
...
- El coste medio:
  ...
  + decrece  cuando $\CMg < \CMe$.

  + crece cuando $\CMg > \CMe$.
